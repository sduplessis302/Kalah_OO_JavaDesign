...
public class Kalah {
    ...
    public void play(IO io) {
        ...
        PrintOutput print = new PrintOutput(io, board);
        print.printBoard();

        for (int i = 0; i < this.numberOfPlayers; i++) {
            if (numberOfPlayers - addRobot <= i) {
                this.player.add(new Robot(io, i+1, board));
            } else {
                this.player.add(new Player(io, i+1));
            }
        }

        String input = this.player.get(currentPlayer-1).promptPlayer();
        ...
        while (true) {
            ...
            input = this.player.get(currentPlayer-1).promptPlayer();
        }
